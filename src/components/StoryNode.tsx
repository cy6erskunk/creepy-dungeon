...
export function StoryNodeInner({
  title,
  banner,
  storyText,
  options,
...
export default function StoryNode({ page }: StoryNodeProps) {
  return (
    <StoryNodeInner
      ...
      banner={page.data.banner as StoryNodeBanner}
      storyText={page.data.storyText}
      options={page.data.options.map(option => ({
        label: option.label ?? '', // Provide empty string as fallback if label is null
        target: option.target && 'uid' in option.target && option.target.uid ? { uid: option.target.uid } : null
      }))}
    />
  );
}
